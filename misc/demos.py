...
from torchvision.models import resnet50, ResNet50_Weights, DenseNet161_Weights, VGG16_BN_Weights, VGG19_BN_Weights, \
    ResNet101_Weights, ResNet18_Weights
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        metadata_features = ['M', 'F', 'Educ1', 'Educ2', 'Educ3', 'Educ4', 'Educ5', 'SES0', 'SES1', 'SES2', 'SES3',
                             'SES4', 'SES5', 'Age']
...
def model_definition(MODEL_NAME):
    ...
    if MODEL_NAME == 'attentioncnn_model':
        model = AttentionCNN()
        model.load_state_dict(torch.load("attentioncnn_model.pt", map_location=device))
    if MODEL_NAME == 'resnet50_model':
        model = ResNet50_w_metadata()
        model.load_state_dict(torch.load("resnet50_model.pt", map_location=device))
    if MODEL_NAME == 'gmlp':
        model = GMLP()
        model.load_state_dict(torch.load("gmlp.pt", map_location=device))
...
def preprocess_data(data):
    ...
    df2 = df1.join(pd.DataFrame(enc.fit_transform(df[['Educ']]).toarray(),
                                columns=('Educ1', 'Educ2', 'Educ3', 'Educ4', 'Educ5'))).drop(columns=('Educ'))
    df3 = df2.join(pd.DataFrame(enc.fit_transform(df[['SES']]).toarray(),
                                columns=('SES0', 'SES1', 'SES2', 'SES3', 'SES4', 'SES5'))).drop(columns=('SES'))

    return df3


def transform_data(train_ds, val_ds, test_ds):
    train_ds, val_ds, test_ds = train_ds.reset_index(drop=True), val_ds.reset_index(drop=True), test_ds.reset_index(
        drop=True)
...
def visualize_grad_cam(activations, gradients, original_image, alpha=0.4):
    ...
    heatmap = F.interpolate(heatmap, size=(original_image.shape[1], original_image.shape[2]), mode='bilinear',
                            align_corners=False)
...
def xai():
    ...
    output = model(test_image, xtabular,
                   register_hook=True)  # Gradients need to be tracked here, so don't use torch.no_grad()
...
def demo_model(test_ds, model, classname="random", filename=None):

    model.eval()
    classmap = {0: "No Dementia", 1: "Very Mild Dementia", 2: "Mild Dementia", 3: "Moderate Dementia"}

    if classname == "random":
        idx = random.choice(range(len(test_ds)))
        image, tabular, label = test_ds[idx]
        image, tabular, label = image.to(device), tabular.to(device), label.to(device)
        image = image.unsqueeze(0)

    #print(image.shape, tabular.shape, label)

    class_label = classmap[torch.argmax(label).item()]
    class_output = classmap[torch.argmax(model(image, tabular)).item()]
    ...
    return image, class_label, class_output

# Assuming you have defined and imported AttentionCNN, test_ds, test_loader

# Define the Grad-CAM visualization function
# def visualize_grad_cam(activations, gradients, original_image, alpha=0.4):
...
#     plt.show()
#
# # Define the demo_model function
# def demo_model(image, tabular, model):
#     model.eval()
#     classmap = {0: "No Dementia", 1: "Very Mild Dementia", 2: "Mild Dementia", 3: "Moderate Dementia"}
#
...
#     image = image.unsqueeze(0)
#
#     output = model(image, tabular, register_hook=True)
#     class_label = classmap[torch.argmax(output).item()]
#
#     target_class = output.argmax(dim=1)
#     model.zero_grad()
...
#     activations = model.get_activations()
#
#     visualize_grad_cam(activations, gradients, image[0])
#
...
if __name__ == '__main__':
    ...
    xdf_data = xdf_data[xdf_data["target"] != 2]
    ...
    MODEL_NAME = "gmlp"
    demo_path = "gmlp_demo"
    model, _, _, _ = model_definition(MODEL_NAME)
    for ii in range(100):
        final_path = os.path.join(demo_path)
        image, class_label, class_output = demo_model(test_ds, model, filename=os.path.join(final_path, f"img{ii}.png"))
        print(f"\t {class_label}\t {class_output}")
        np.save(os.path.join(final_path,f"labels{ii}.npy"), [class_label, class_output])
        print(f"Finished {ii + 1}")
